...
def get_numeric_columns(df: pd.DataFrame) -> List[str]:
    ...
    logger.debug(f"Found {len(numeric_columns)} numeric columns: {numeric_columns}")
...
def calculate_statistics(data: pd.Series) -> Dict:
    ...
    Parameters:
        data (pd.Series): Numeric data to analyze
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if col_idx < 0 or col_idx >= len(numeric_columns):
                    print("Invalid column number. Please try again.")
                    continue
                
                selected_column = numeric_columns[col_idx]
