...
def display_results(stats: Dict, column: str) -> None:
    """
    Displays calculated statistics for a column.
...
def process_file(file_path: str) -> None:
    ...
    try:
        ...
        while True:
            try:
                selection = int(input("\nEnter the number of the column to analyze (0 to exit): "))
                if selection == 0:
                    break
                if 1 <= selection <= len(numeric_columns):
                    selected_column = numeric_columns[selection - 1]
